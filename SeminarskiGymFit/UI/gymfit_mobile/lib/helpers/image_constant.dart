class ImageConstant {
  // Image folder path
  static String imagePath = 'assets/images';
  static String iconPath = 'assets/icons';

 
  static String imgStart = '$imagePath/startImage.jpg';

 
  static String img1858113176x390 = '$imagePath/img_1858113_1_76x390.png';

  static String imgtrainer = '$imagePath/trainer.jpg';

  static String imgCall = '$imagePath/img_call.svg';

  static String imgGettyimages104 = '$imagePath/img_gettyimages_104.png';

  static String imgImages11 = '$imagePath/img_images_1_1.png';

  static String imgIstockphoto856797530612x612 =
      '$imagePath/img_istockphoto_856797530_612x612.png';

  static String imgIstockphoto132 = '$imagePath/img_istockphoto_132.png';

  static String imgRectangle4163 = '$imagePath/img_rectangle_4163.png';

  static String imgGroupAddLight = '$imagePath/img_group_add_light.svg';

  static String img1858113174x390 = '$imagePath/img_1858113_1_74x390.png';

  static String imgVector15 = '$imagePath/img_vector_15.svg';

  static String imgRectangle41648x387 =
      '$imagePath/img_rectangle_4164_8x387.png';

  // user profile images
  static String imgSignOutSqure = '$imagePath/img_sign_out_squre.svg';

  static String imgRectangle41641 = '$imagePath/img_rectangle_4164_1.png';

  // my reservations images
  static String img1858113177x390 = '$imagePath/img_1858113_1_77x390.png';

  static String imgFlag = '$imagePath/img_flag.svg';

  // notifications images
  static String imgRectangle41642 = '$imagePath/img_rectangle_4164_2.png';

  static String imgClose = '$imagePath/img_close.svg';

  static String imgEdit = '$imagePath/img_edit.svg';

  static String imgEditPrimarycontainer =
      '$imagePath/img_edit_primarycontainer.svg';

  // Common images
  static String logo = '$imagePath/logo.png';

  static String backImage = '$imagePath/backImage.jpg';

  static String logo1 = '$imagePath/logo1.png';

  static String img18581131 = '$imagePath/img_1858113_1.png';

  static String imgBellNotificationOn02 =
      '$imagePath/img_bell_notification_on_02.svg';

  static String gym1 = '$imagePath/gym1.jpg';

  static String gym2 = '$imagePath/gym3.jpg';

  static String gym3 = '$imagePath/gym2.jpg';

  static String gallery1 = '$imagePath/gallery.jpg';
  static String gallery2 = '$imagePath/gallery2.jpg';
  static String gallery3 = '$imagePath/gallery3.jpg';
  static String gallery4 = '$imagePath/gallery4.jpg';
  static String gallery5 = '$imagePath/gallery5.jpg';
  static String gallery6 = '$imagePath/gallery6.jpg';
  static String gallery7 = '$imagePath/gallery7.jpg';
  static String gallery8 = '$imagePath/gallery8.jpg';
  static String gallery9 = '$imagePath/gallery9.jpg';
  static String gallery10 = '$imagePath/gallery10.jpg';
  static String gallery11 = '$imagePath/gallery11.jpg';
  static String gallery12 = '$imagePath/gallery12.jpg';
  static String gallery13 = '$imagePath/gallery13.jpg';

  static String imgMohamedFareed = '$imagePath/img_mohamed_fareed.png';

  static String imgSushilGhimire = '$imagePath/img_sushil_ghimire.png';

  static String imgBrettJordanU2 = '$imagePath/img_brett_jordan_u2.png';

  static String imgHumphreyMuleba = '$imagePath/img_humphrey_muleba.png';

  static String imgRodrigoS2mz9i = '$imagePath/img_rodrigo_s_2mz9i.png';

  static String imgVector9 = '$imagePath/img_vector_9.svg';

  static String imgUser = '$iconPath/user.svg';

  static String imgReservation = '$iconPath/reservation.svg';

  static String imgMyReservation = '$iconPath/myReservation.svg';

  static String imgHome = '$iconPath/home.svg';

  static String imgRectangle4164 = '$imagePath/img_rectangle_4164.png';

  static String trainers= '$iconPath/trainers.svg';

  static String img18581131103x390 = '$imagePath/img_1858113_1_103x390.png';

  static String imgBillGatesProfilePic1 =
      '$imagePath/img_bill_gates_profile_pic_1.png';

  static String imageNotFound = 'assets/images/image_not_found.png';
}